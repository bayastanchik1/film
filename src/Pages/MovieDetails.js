...
}} src={`https://www.themoviedb.org/t/p/w300_and_h450_bestv2${movie.poster_path}`} alt="img" /> 
<div className="text">
    <h1><span>Name : </span>{movie.title}</h1>
    <h3> <span>Popularity : </span>{movie.popularity}</h3>
</div>
</div>
                }
            ...
            { movie.length === 0 ? <img src={zag} alt="img" /> :
                <img onMouseMove={() => setHover(true)} onMouseOut={() => setHover(false)} onClick={() => setBlo(true) } src={`https://www.themoviedb.org/t/p/w300_and_h450_bestv2${movie.poster_path}`} alt="img" />
}
{

         movie.length === 0 ?  <img  style={{marginLeft: "100px"}} src={texr} alt="img" /> :  <div className="details-text">
                <h1>{movie.title}</h1>
              <div className="hh33">
              <h4 style={{
                    margin:"10px 0"
                }}>{movie.release_date} <span>({movie.original_language})</span></h4>
                {movie.genres?.map((el) => (
                    <h3>{el.name}</h3>
                ))}
                <h3>{Math.floor(movie.runtime / 60)}h  {" "} {Math.round(movie.runtime % 60)}h</h3>
              </div>
...
<h3 className='h'>{movie.tagline}</h3>
            <h1>{Math.round(movie.vote_average * 10)}% <span>Рейтинг</span></h1>
                <h2 style={{
                    margin:"20px 0"
                }}>Обзор</h2>
